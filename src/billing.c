...
double calculateWaitTime(int specialtyQueueCount[NUM_SPECIALTIES], int specialtyId, int consultTimes[NUM_SPECIALTIES]) {
    double waitTime = specialtyQueueCount[specialtyId - 1] * consultTimes[specialtyId - 1];
...
void printBillReceipt(char *patientId, char *patientName, int age,
    ...
    double waitTime = calculateWaitTime(specialtyQueueCount, specialtyId, consultTimes);

    double surchargePercent = (urgencyLevel == 2) ? 20.0 : (urgencyLevel == 3) ? 50.0 : 0.0;
